...
class TaskDB(BaseTaskDB):
    ...
    def projects(self):
        if time.time() - getattr(self, '_last_update_projects', 0) \
            ...
            self._projects = set(utils.text(x) for x in
                    self.redis.smembers(self.__prefix__ + 'projects'))
    ...
    def get_task(self, project, taskid, fields = None):
        if fields:
            ...
            obj = self.reids.hgetall(self._gen_key(project, taskid))
    ...
    def insert(self, project, taskid, obj={}):
        ...
        if project not in self.projects:
            pipe.sadd(self.__prefix__ + 'project', project)
    ...
    def drop(self, project):
        ...
        for each in itertools.tee(scan_method("%s%s_*" % (self.__prefix__,
            project)), 100):
            each = list(each)
            if each:
